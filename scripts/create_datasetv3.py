...
def plot_removal_summary(global_nan_percentages: dict, vars_to_remove: dict, 
    ...
    plt.yticks(range(len(vars_sorted)), vars_sorted, fontsize=48)
    plt.xlabel('Global NaN Percentage (%)', fontsize=48)
    ...
    plt.yticks(range(len(all_vars_sorted)), all_vars_sorted, fontsize=48)
    plt.xlabel('Global NaN Percentage (%)', fontsize=48)
    plt.ylabel('Variable', fontsize=48)
